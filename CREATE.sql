...
AS
BEGIN
    SELECT EMP.First_Name + SPACE(1) + EMP.Last_Name AS Employee_Full_Name
    From EMPLOYEE EMP
    WHERE EMP_ID = (
        SELECT TOP 1
        Employee_ID
...
-- TRIGGER 3
CREATE TRIGGER OnUpdateOrderAmountOrderReceipt
   ON  [ORDER]
  AFTER UPDATE
AS 
BEGIN
    IF UPDATE(Amount) -- only interested in specific change
  BEGIN

        DECLARE @Order_ID INT
        DECLARE @Order_Amount Money
        DECLARE @Discount_Rate INT
        DECLARE @Receipt_Amount INT


        SELECT @Order_ID = Order_ID from inserted;
        SELECT @Order_Amount = Amount from inserted;

        SELECT @Discount_Rate = Discount from RECEIPT WHERE Order_ID = @Order_ID;

        IF(@Discount_Rate IS NULL)
BEGIN
            SET @Discount_Rate= 0;
            SET @Receipt_Amount = (@Order_Amount + (0.07*@Order_Amount));
            UPDATE RECEIPT SET Tax = (@Order_Amount * 0.07), Amount = @Receipt_Amount, Discount = NULL WHERE Order_ID = @Order_ID;
        END
ELSE 
BEGIN
            -- SET @Discount_Rate= 0;
            SET @Receipt_Amount = (@Order_Amount + (0.07*@Order_Amount) - ((@Discount_Rate*@Order_Amount)/100));
            UPDATE RECEIPT SET Tax = (@Order_Amount * 0.07), Amount = @Receipt_Amount WHERE Order_ID = @Order_ID;
        END
    END
END
GO
